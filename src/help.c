...
const struct HelpPage pages[helpPageCount] = {
    ...
    {"String",NULL,page_basic,"printing,quotes,text","Strings are values that hold text. Strings are written between two quote marks (\"). They can be concatenated using the add function or the plus operator. An example would be, <syntax>\"hello \"+\"world\"</syntax>. When other values are added to strings, they are converted to strings. For example <syntax>reports=&gt;\"report count: \"+reports</syntax>. <br>The functions that support strings are:<ul><li><help>equal</help></li><li><help>comparisons</help></li><li><help>add</help> (+)</li><li><help>length</help></li><li><help>strring</help></li><li><help>eval</help></li><li><help>print</help></li><li><help>error</help></li><li><help>replace</help></li><li><help>indexof</help></li><li><help>substr</help></li><li><help>lowercase</help></li><li><help>uppercase</help></li></ul> See also: <help>escape sequence</help>"},
    {"Escape sequence",NULL,page_basic,"newline,new line,progress bar,carriage return,quote,string,text","Escape sequences are special characters that cannot be written in plain text. They start with a all start with a backslash and are only valid within strings.<br>List:<table><tr><td>Sequence</td><td>Name</td></tr><tr><td>\\n      </td><td>New line</td></tr><tr><td>\\\\      </td><td>Backslash</td></tr><tr><td>\\r      </td><td>Carriage return</td></tr><tr><td>\\t      </td><td>Tab</td></tr><tr><td>\\\"      </td><td>Quote</td></tr></table><br>To properly write quotes within a string, a backslash is needed to prevent the termination of the string. Since backslashes are needed as an escape character, real backslashes within a string require two backslashes. All other characters are useful for printing. For example <syntax>\"\\n\"</syntax> inserts a line break, <syntax>\"\\r\"</syntax> moves the cursor to the beginning of the line, and <syntax>\"\\t\"</syntax> inserts a tab character."},
    ...
    {"Local variables", NULL,page_basic,"variables,keys,accessors","Local variables are temporary variables used to store values. As opposed to custom functions with no inputs, local variables store a value, not an expression. Local variables are defined with an name, an equal sign, and an expression. The name must be at the start of the line, they cannot be set within expressions. Local variables can also be used within <help>multiline functions</help> and will stay contained within each block. If the local variable name is already taken, the program will overwrite the old one. Altering local variables is also possible with keys. For example, <help>vectors</help> and <help>strings</help> can be set using <syntax>var[index]=value</syntax>, where index is a complex number representing the coordinates of the cell. However, accessors cannot yet be used to read values, use <syntax><help>ge</help></syntax>. To view local variables, run the <help title='-ls'><syntax>-ls local</syntax></help> function. <br>Examples:<ul><li><syntax>x=sqrt(4-4*2*3)</syntax></li><li><syntax>gamma=x=>fact(x-1)</syntax></li></ul>"},
    #pragma endregion
    ...
    {"Argument","arg(z)",page_function,"angle,imaginary","<syntax>arg(z)</syntax> returns the argument of <syntax>z</syntax>. The argument of a complex number is the angle from the positive real axis.<br><h3>Examples:</h3><br><syntax>arg(1) = 0</syntax><br><syntax>arg(i) = pi/2</syntax><br><syntax>arg(-2) = pi</syntax>"},
    #pragma endregion
    ...
    {"Get real","getr(z)",page_function,"component","<syntax>getr(z)</syntax> returns <syntax>z</syntax> stripped of the imaginary and unit components. For vectors, a vector is returned with <syntax>getr</syntax> called on each element."},
    {"Get imaginary","geti(z)",page_function,"component","<syntax>geti(z)</syntax> returns <syntax>z</syntax> stripped of the real and unit components. For vectors, a vector is returned with <syntax>geti</syntax> called on each element."},
    {"Get unit","getu(z)",page_function,"component","<syntax>getu(z)</syntax> returns <syntax>z</syntax> stripped of the numeric components. That means it returns 1 multiplied by <syntax>z</syntax>'s <help>unit</help>. For vectors, a vector is returned with <syntax>getu</syntax> called on each element."},
    #pragma endregion
    ...
    {"Typeof","typeof(a)",page_function,NULL,"<syntax>typeof(a)</syntax> returns the type of <syntax>a</syntax>.<table><tr><td>Type               </td><td>Id</td></tr><tr><td>Number             </td><td>0</td></tr><tr><td><help>Vector             </help></td><td>1</td></tr><tr><td><help>Anonymous function </help></td><td>2</td></tr><tr><td>Arbitrary precision</td><td>3</td></tr><tr><td><help>String             </help></td><td>4</td></tr></table>"},
    {"History","hist(n)",page_function,"previous answer","<strong>hist(n)</strong> returns the <em>n</em>th value in the history. The alternative syntax for hist $ notation. <syntax>$1</syntax> is equivalent to <syntax>hist(1)</syntax>, and <syntax>$-2</syntax> is equivalent to <syntax>hist(-2)</syntax>. For negative <em>n</em>, <em>hist</em> returns <syntax>hist(<help>histnum</help>+n)</syntax>. For example, <syntax>hist(-1)</syntax> returns the previous value. Anything outside of the range of the calculation history will give an error."},
    ...
    {"Sum","sum(func,start,end,step)",page_function,"summation,addition","<strong>sum(func,start,end,step)</strong> returns the summation of a series. It is exactly identical to this script:<br><strong> out = 0<br>for(i=start;i&lt;end;i+=step) out+=run(func,i);<br>return out;<br></strong>The last argument <syntax>step</syntax> is optional and will default to 1. This function supports vectors as outputs."},
    {"Product","product(func,start,end,step)",page_function,"multiplication","<strong>product(func,start,end,step)</strong> returns the product of a series. It is exactly identical to this script:<br><strong> out = 1<br>for(i=start;i&lt;end;i+=step) out*=run(func,i);<br>return out;<br></strong>The last argument <syntax>step</sytnax> is optional and will defualt to 1. This function supports vectors as outputs."},
    ...
    {"Length", "length(val)",page_function,"element count,vector,matrix,string","For vectors, <syntax>length(vec)</syntax> returns the total number of elements in <syntax>vec</syntax>. It is equivalent to <sytnax><help>height</help>(vec)*<help>length</help>(vec)</syntax>. For strings, <syntax>length(str)</syntax> returns the total number of characters in the string. For all other values, one is returned."},
    ...
    {"String","string(val,base)",page_function,"convert","<syntax>string(val,base)</syntax> or <syntax>string(val)</syntax> will convert the value <syntax>val</syntax> to a string type. The <syntax>base</syntax> argument is optional and must be an integer from 2 to 36."},
    {"Evaluate","eval(str,base)",page_function,NULL,"<syntax>eval(str,base)</syntax> or <syntax>eval(str)</syntax> will evaluate a string type as if it is an expression. This allows the composition of expressions, allowing more flexible custom functions, but it is also error prone. <help>Anonymous functions</help> are better for flexible programming, but do not have the same power. The <syntax>base</syntax> argument is optional and determines the base <syntax>str</syntax> will be parsed in."},
    {"Print","print(str)",page_function,NULL,"<syntax>print(str)</syntax> is a function that will convert <syntax>str</syntax> to a string (if it is not already) and print it in the output feed. Print does not append line breaks to the end of the line, which means <syntax>print(\"a\");print(\"b\")</syntax> and <syntax>print(\"a\"+\"b\")</syntax> have the same behavior. However after an expresssion is done computing, a new line is added if the previous print call did not end with one. This means the return value will always be on a new line."},
    {"Error","error(str)",page_function,NULL,"<syntax>error(str)</syntax> will print an error message and send an error signal. This means that in a multiline function, script execution will stop. This can be used in multiline functions to prevent code from executing or to block certain value types in arguments. By default, <syntax>error</syntax> will automatically write \"Error:\" at the beginning of the message (colored red). Note that in the order of execution, the statements within the call will be evaluated first, so if an error occurs within there, this function will not be called."},
    {"Replace","replace(str,old,new,count)",page_function,NULL,"<syntax>replace(str,old,new,count)</syntax> will return <syntax>str</syntax> with all occurences of <syntax>old</syntax> replaced with <syntax>new</syntax> up to <syntax>count</syntax> times. <syntax>count</syntax> is an optional integer.<br>Examples:<ul><li><syntax>replace(\"hello world\",\"o\",\"a\") = \"hella warld\"</syntax></li><li><syntax>replace(\"hello world\",\"o\",\"a\",1) = \"hella world\"</syntax></li></ul>"},
    {"Index of","indexof(str,find,start)",page_function,"string,position","<syntax>indexof(str,find,start)</syntax> will return the first position of the string <syntax>find</syntax> within <syntax>str</syntax>. Index 0 is the first character. <syntax>start</sytnax> is an optional integer argument that says to ignore the first <syntax>start</syntax> characters.<br>Examples:<ul><li><syntax>indexof(\"calculator\",\"a\") = 1</syntax></li><li><syntax>indexof(\"calculator\",\"l\") = 2</syntax></li><li><syntax>indexof(\"calculator\",\"l\",3) = 5</syntax></li></ul>"},
    {"Substring","substr(str,start,end)",page_function,"section","<syntax>substr(str,start,end)</syntax> will return a subsection of <syntax>str</syntax> from the position <syntax>start</syntax> up to, but not including, <syntax>end</syntax>. <syntax>end</syntax> is optional and will default to the length of <syntax>str</syntax>. If either of the positional arguments are negative, they will wrap around, with negative one being the last character of the string.<br>Examples:<ul><li><syntax>substr(\"calculator\",2) = \"lculator\"</syntax></li><li><syntax>substr(\"calculator\",0,4) = \"calc\"</syntax></li><li><syntax>substr(\"calculator\",-2) = \"or\"</syntax></li></ul>"},
    {"Lowercase","lowercase(str)",page_function,NULL,"<syntax>lowercase(str)</syntax> will return <syntax>str</syntax> with all of the characters A-Z replaced with a-z."},
    {"Lowercase","uppercase(str)",page_function,NULL,"<syntax>uppercase(str)</syntax> will return <syntax>str</syntax> with all of the characters a-z replaced with A-Z."},
    #pragma endregion
    #pragma endregion
    #pragma region Units
